 ...
 * S    - kernel status field, p_rts_flags; mm status field, mp_flags (R if p_rt
s_flags
...
/* This block is from who.c */
#include <utmp.h>
#include <time.h>
#include <minix/paths.h>
/* End from who.c (doubles removed) */
 ...
 * USER    TTY     LOGIN@  WHAT  
 * uuuuuu  tttttt  hh:ss   cccccccccc...
 *
...
/* The following two lines have been changed. */
#define S_HEADER "USER      TTY       LOGIN@ WHAT\n"
#define S_FORMAT "%8s %6s %s %s\n" /* Reset this line to original. */
...
/* The who.c while loop can go here: */
...
/* Merge printf statements from who.c and ps.c */
            if (strstr(ut.ut_line, tname((dev_t) buf.ps_dev)))
              printf("%-9.8s %-9.8s %02d:%02d  %s\n",
                        ut.ut_name,
                        ut.ut_line,
                        tm->tm_hour,
                        tm->tm_min,
